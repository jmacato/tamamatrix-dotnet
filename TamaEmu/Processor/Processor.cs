...
namespace TamaEmu.Processor
{
	...
	public abstract class Processor
	{
		...
		protected void SetDisassembly()
        {
			...
			Logger.Log($"{ProgramCounter.ToString("X").PadLeft(4, '0')} : {CurrentOpCode.ToString("X").PadLeft(2, '0')}{CurrentDisassembly.LowAddress}{CurrentDisassembly.HighAddress} {CurrentDisassembly.OpCodeString} {CurrentDisassembly.DisassemblyOutput.PadRight(10, ' ')} A: {Accumulator.ToString("X").PadLeft(3, '0')} X: {XRegister.ToString("X").PadLeft(3, '0')} Y: {YRegister.ToString("X").PadLeft(3, '0')} SP {StackPointer.ToString("X").PadLeft(3, '0')} N: {Convert.ToInt16(NegativeFlag)} V: {Convert.ToInt16(OverflowFlag)} B: {0} D: {Convert.ToInt16(DecimalFlag)} I: {Convert.ToInt16(DisableInterruptFlag)} Z: {Convert.ToInt16(ZeroFlag)} C: {Convert.ToInt16(CarryFlag)}");
		}
